...
function Web3Connector (chainConfig, mschub, connectionCallback) {
  ...
  this.web3.setProvider(
    new this.web3.providers.HttpProvider(chainConfig.rpcServer)
  )
  ...
  window.setInterval(
    function () {
      ...
      try {
        ...
        this.getAllAccountDetails().then(
          accounts => (mschub.financialData.accounts = accounts)
        )
      } catch (e) {
      ...
      if (wasConnected !== this.connected) {
        console.log(
          'web3 connection status changed: ' + JSON.stringify(newStatus)
        )
...
Web3Connector.prototype.getNodeId = function () {
  return this.rpcCall('admin_nodeInfo', [])
    .then(nodeInfo => {
      ...
      console.log(
        'getNodeId did not return an enode id, response from gmc was: ' +
          nodeInfo
      )
      return 'Not Found'
    })
...
Web3Connector.prototype.getAllAccountDetails = function () {
  ...
  const output = accounts.map(account =>
    this.getAccountDetails(account, coinbase).then(details => {
  ...
  if (accounts.indexOf(coinbase) < 0) {
    output.push(
      this.getAccountDetails(coinbase, coinbase).then(details => {
...
Web3Connector.prototype.getAccountDetails = function (account, _coinbase) {
  const coinbase = _coinbase || this.getCoinbase()
  return this.getUserBalanceInMusicoin(account).then(balance => {
    return {
      address: account,
      avatar: blockies
        .create({
  ...
  return this.unlockAccount(from, pwd)
    ...
    .then(function (account) {
      var params = {
        to: recipient,
        from: account,
        value: this.toIndivisibleUnits(coins),
        gas: 940000
      }
      return new Promise(
        function (resolve, reject) {
          ...
          this.web3.eth.getTransactionReceipt(
            ...
            function (error, receipt) {
              if (receipt && receipt.transactionHash === expectedTx) {
                ...
                this.web3.eth.getTransaction(expectedTx, function (
                  error,
                  transaction
                ) {
