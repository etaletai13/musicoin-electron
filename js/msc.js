...
var pathOfNodes = musicoinRoot + '/config/bootnodes.json'
var configFolderHome = musicoinRoot + '/config'
...
if (!fs.existsSync(pathOfNodes)) {
  fs.copySync(process.cwd() + '/config/bootnodes.json', pathOfNodes)
}
if (!fs.existsSync(configFolderHome)) {
  fs.copySync(configFolder, configFolderHome)
}
if (!fs.existsSync(settings)) {
  fs.copySync(configFolder + '/settings.js', settings)
}

if (fs.existsSync(musicoinRoot + '/chaindata')) {
  fs.remove(musicoinRoot + '/chaindata')
}
if (fs.existsSync(musicoinRoot + '/dapp')) fs.remove(musicoinRoot + '/dapp')
if (fs.existsSync(musicoinRoot + '/nodekey')) {
  fs.remove(musicoinRoot + '/nodekey')
}
...
var web3Connector = new Web3Connector(settings.chain, mschub, function (
  connected,
  err
) {
  if (err) {
